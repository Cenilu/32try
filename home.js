document.addEventListener('DOMContentLoaded', function () {
    ...
    var zoomedOnce = false; // Flag to track whether zoom has been performed
var myLocationMarker = null; // Initialize myLocationMarker outside the function
var btnStartClicked = false; // Flag to track whether btnStart has been clicked
...
function locateUser() {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.watchPosition(function (position) {
            ...
            if (!zoomedOnce && !btnStartClicked) { // Check if zoom has not been performed yet and btnStart has not been clicked
                map.setView(userLocation, 14); // Zoom to user location only if not zoomed yet
                zoomedOnce = true; // Set the flag to true after zooming
            }

            if (btnStartClicked) { // Check if btnStart has been clicked
                map.setView(userLocation, map.getZoom()); // Keep focusing on user location if btnStart has been clicked
            }
    ...
    document.getElementById("btnStart").addEventListener("click", function () {
        btnStartClicked = true; // Set the flag to true when btnStart is clicked
        ...
        if (routingControl) {
            ...
            if (myLocationMarker && myDestinationMarker) {
                ...
                // Zoom to user location marker if btnStart is clicked
